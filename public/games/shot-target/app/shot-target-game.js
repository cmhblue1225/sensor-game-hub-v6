...
export class ShotTargetGame {
    ...
    handleNormalSensorConnected(data) {
        ...
        // 플레이어 객체 생성 및 추가 (중요!)
        if (!this.sensorManager.players.has(sensorId)) {
            const colorIndex = sensorId === 'sensor1' ? 0 : 1;
            const player = new Player(sensorId, null, colorIndex);
            
            // 조준점 초기 위치 설정 (화면 중앙)
            player.crosshair.x = this.canvas.width / 2;
            player.crosshair.y = this.canvas.height / 2;
            player.crosshair.targetX = player.crosshair.x;
            player.crosshair.targetY = player.crosshair.y;
            
            this.sensorManager.addPlayer(sensorId, player);
            this.scoringSystem.initializePlayer(sensorId, player.name);
            
            console.log(`✅ 플레이어 생성: ${sensorId} (${player.name})`);
        }
    ...
    handleSensorData(data) {
        ...
        console.log('📡 센서 데이터 수신:', {
            ...
            hasPlayers: this.sensorManager.players.size
        });
        
        // 센서 매니저에 데이터 전달
        this.sensorManager.processSensorData(data.sensorId, data.data);
        
        // 항상 조준점 업데이트 (게임 시작 전에도)
        this.sensorManager.updateAllCrosshairs(this.canvas.width, this.canvas.height);
        this.updateDOMCrosshair();
        
        // 강제로 한 번 렌더링 (게임 시작 전에도 조준점 보이도록)
        this.render();
        
        // 임시 테스트: DOM 조준점을 센서 데이터로 직접 움직이기
        this.testDirectCrosshairMovement(data);
    }

    // 임시 테스트: 센서 데이터로 DOM 조준점 직접 움직이기
    testDirectCrosshairMovement(data) {
        const crosshairElement = document.getElementById('crosshair');
        if (!crosshairElement || !data.data?.orientation) return;
        ...
        const x = centerX + (gamma * sensitivity);
        const y = centerY + (beta * sensitivity);
        
        // 화면 경계 제한
        const clampedX = Math.max(0, Math.min(this.canvas.width, x));
        const clampedY = Math.max(0, Math.min(this.canvas.height, y));
        
        crosshairElement.style.left = `${clampedX}px`;
        crosshairElement.style.top = `${clampedY}px`;
        crosshairElement.classList.remove('hidden');
        
        console.log('🎯 직접 조준점 이동:', {
            beta, gamma,
            x: clampedX, y: clampedY
        });
    }

    // DOM 조준점 업데이트 (HTML 요소)
    updateDOMCrosshair() {
        const crosshairElement = document.getElementById('crosshair');
        if (!crosshairElement || !this.sensorManager) return;

        // 메인 플레이어의 조준점 위치 가져오기
        const mainPlayer = this.sensorManager.players.get('sensor1') || 
                          Array.from(this.sensorManager.players.values())[0];
        
        if (mainPlayer) {
            crosshairElement.style.left = `${mainPlayer.crosshair.x}px`;
            crosshairElement.style.top = `${mainPlayer.crosshair.y}px`;
            crosshairElement.classList.remove('hidden');
        }
    }
