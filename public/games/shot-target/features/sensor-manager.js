...
export class SensorManager {
    ...
    processSensorData(sensorId, data) {
        ...
        // 플레이어 센서 데이터 업데이트 (스로틀링 제거 - 즉시 처리)
        if (this.players.has(sensorId)) {
            ...
            if (data.orientation) {
                ...
                // 디버그 로그 추가
                console.log(`🔍 센서 데이터 처리: ${sensorId}`, {
                    beta: data.orientation.beta,
                    gamma: data.orientation.gamma,
                    playerTilt: { x: player.tilt.x, y: player.tilt.y }
                });
            }
        } else {
            console.warn(`⚠️ 플레이어 없음: ${sensorId}`);
        }
    ...
    updateAllCrosshairs(canvasWidth, canvasHeight) {
        console.log(`🎯 [센서 매니저] 조준점 업데이트 시작 - 플레이어 수: ${this.players.size}`);
        
        this.players.forEach((player, playerId) => {
            if (!player.isActive || !player.isConnected) {
                console.log(`⚠️ [센서 매니저] 비활성 플레이어 스킵: ${playerId}`);
                return;
            }
            ...
            console.log(`🎯 [센서 매니저] ${playerId} 조준점 업데이트 - 모드: ${screenMode}`);
