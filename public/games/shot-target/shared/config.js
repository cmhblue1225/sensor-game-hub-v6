// ===== SHARED/CONFIG =====
// 게임 설정 및 상수 정의

export const GAME_CONFIG = {
    // 게임 모드
    MODES: {
        SOLO: 'solo',
        COOP: 'coop',
        COMPETITIVE: 'competitive',
        MASS_COMPETITIVE: 'mass-competitive'
    },

    // 표적 타입 설정
    TARGET_TYPES: {
        large: {
            radius: 60,
            points: 100,
            color: '#ef4444',
            spawnChance: 0.4
        },
        medium: {
            radius: 40,
            points: 200,
            color: '#f59e0b',
            spawnChance: 0.4
        },
        small: {
            radius: 25,
            points: 500,
            color: '#10b981',
            spawnChance: 0.2
        }
    },

    // 게임 플레이 설정
    GAMEPLAY: {
        targetLifetime: 5000,           // 5초 후 자동 소멸
        targetSpawnInterval: 2000,      // 2초마다 새 표적 생성 (기본값)
        hitRadius: 15,                  // 조준점이 표적 중심에서 이 거리 내에 있으면 발사
        comboMultiplier: 1.5,
        bulletSpeed: 8,
        maxTargets: 3,                  // 최대 동시 표적 수 (기본값)
        gameTimeDefault: 180,           // 3분 = 180초 (기본값)
        gameTimeMassCompetitive: 120    // 2분 = 120초 (대규모 경쟁)
    },

    // 센서 설정 (개선된 부드러운 움직임)
    SENSOR: {
        sensitivity: 15,                // 센서 감도
        maxTilt: 25,                   // 최대 기울기 각도
        smoothing: {
            solo: 0.15,                // 솔로 모드 스무딩 (더 부드럽게)
            coop: 0.12,                // 협동 모드 스무딩
            competitive: 0.18,         // 경쟁 모드 스무딩
            massCompetitive: 0.14      // 대규모 경쟁 모드 스무딩 (끊김 방지)
        },
        throttleInterval: 16           // 60fps를 위한 16ms 간격 (기존 33ms에서 개선)
    },

    // 대규모 경쟁 모드 전용 설정
    MASS_COMPETITIVE: {
        baseTargets: 2,                // 기본 표적 수 (3명 기준)
        targetsPerPlayer: 1,           // 플레이어 1명당 추가 표적 수
        maxTargetsLimit: 12,           // 절대 최대 표적 수 (8명 * 1.5)
        baseSpawnInterval: 1500,       // 기본 생성 간격 (더 빠름)
        minSpawnInterval: 800,         // 최소 생성 간격 (너무 빨라지지 않게)
        spawnIntervalReduction: 100,   // 플레이어 1명당 간격 단축
        minPlayers: 3,                 // 최소 플레이어 수
        maxPlayers: 8                  // 최대 플레이어 수
    },

    // 플레이어 색상 팔레트
    PLAYER_COLORS: [
        '#3b82f6', '#ef4444', '#10b981', '#f59e0b',
        '#8b5cf6', '#06b6d4', '#f97316', '#84cc16'
    ],

    // UI 설정
    UI: {
        crosshairSize: 30,
        effectDuration: 500,
        popupDuration: 1500,
        modalTransition: 300
    }
};

// SDK 설정
export const SDK_CONFIG = {
    gameId: 'shot-target',
    debug: true,
    getGameType: (mode) => {
        switch (mode) {
            case GAME_CONFIG.MODES.SOLO:
                return 'solo';
            case GAME_CONFIG.MODES.MASS_COMPETITIVE:
                return 'multi';
            default:
                return 'dual';
        }
    }
};